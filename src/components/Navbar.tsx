...
import { Download, Menu, X, BarChart2, Sun, Moon } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onOpenCvModal, theme, onToggleTheme }) => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
        isScrolled
          ? 'glass-nav py-3 shadow-xl shadow-slate-900/10 dark:shadow-slate-950/40'
          : 'bg-slate-900/90 dark:bg-slate-950/90 backdrop-blur-md py-5 border-b border-slate-800/80'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <a href="#home" className="flex items-center gap-3 group">
          ...
          <div>
            <span className="font-extrabold text-base sm:text-lg text-white dark:text-white tracking-tight group-hover:text-sky-400 transition-colors">
        ...
        {/* Desktop Nav Links */}
        <nav className="hidden lg:flex items-center gap-1 bg-slate-800/60 dark:bg-slate-900/60 p-1.5 rounded-full border border-slate-700/50">
          {navLinks.map((link) => (
            <a
              key={link.id}
              href={link.href}
        ...
        {/* Actions (Theme Toggle + Download CV) */}
        <div className="hidden sm:flex items-center gap-3">
          {/* Light / Dark Mode Toggle Button */}
          <button
            onClick={onToggleTheme}
            className="p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white border border-slate-700 transition-all cursor-pointer flex items-center justify-center"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-slate-900/95 dark:bg-slate-950/95 backdrop-blur-xl border-b border-slate-800 px-4 pt-3 pb-6 space-y-2 animate-fadeIn">
          {navLinks.map((link) => (
            <a
              key={link.id}
              href={link.href}
          ...
          <div className="pt-2 flex flex-col gap-2">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 text-slate-200 text-xs font-semibold"
            >
